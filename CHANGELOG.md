...
- **Coverage now traces inside `ProcessPoolExecutor` workers.** `toys.py`
  ...
  single test file, changing nothing but the setting, moved `toys.py` from 26%
  to 37% without a single new test being written. This is the same class of
  error as the Numba problem `scripts/run_coverage.sh` exists for, and the more
  dangerous of the two: the Numba figure was absurd enough to force a second
  look, whereas a plausible 62% instead invites someone to write tests for code
