...
The test suite currently comprises 247 tests across 29 files under `tests/`, grouped into four categories. A default environment runs 228 of them: 18 are UltraNest-specific and skip automatically if the optional `ultranest` package isn't installed, and one of the two `extra_nll` Numba-guard tests always skips, since each is scoped to the JIT mode it describes and only one of those can hold at a time. Under `NUMBA_DISABLE_JIT=1` — which `scripts/run_coverage.sh` and the CI Coverage job both set for most of the suite — the other one runs instead, and three `pyfc.priors` tests skip, because `@njit` returns a plain function in that mode so the jitted/plain distinction they describe does not exist. The suite therefore never reports all 247 passing in any single run, by design.
...
│   └── 08_pyfc_soft_constraints_tutorial.ipynb       # Soft constraints via extra_nll/pyfc.priors: Gaussian, correlated, one-sided
...
The `examples/` directory contains eight runnable Jupyter notebooks, numbered `01`-`08` in the
...
capability. `06` and `08` are the two halves of the constraint story -- hard geometry and
soft priors respectively -- and are best read together. See also `docs/source/tutorials.rst` (rendered as part of the [hosted
...
so an unpicklable `extra_nll` makes that pool fall back to threads — correct, but slower.
What is unpicklable is an **un-jitted** lambda or closure; a jitted one pickles fine, since a
numba dispatcher serialises itself. Measured: a module-level function, a `gaussian_block`
closure and a nested `combine_priors` result all survive a real subprocess round-trip, while
an un-jitted lambda raises. So define an *un-jitted* penalty at module level; a builder's
output needs no such care.

[`examples/08_pyfc_soft_constraints_tutorial.ipynb`](https://github.com/mbustama/FeldmanCousins/blob/main/examples/08_pyfc_soft_constraints_tutorial.ipynb)
...
* **A covariance that isn't positive definite is accepted by `np.linalg.inv`.** The penalty
  ...
  naming the smallest eigenvalue, the tolerance and the condition number. The test is by
  eigenvalue rather than by `np.linalg.cholesky`, which is not reliable here: at
  `sigma = (0.2, 0.3)` with correlation exactly 1, `0.04*0.09` and `0.06**2` differ in the
  last bit, the determinant lands at `+1e-18`, and cholesky accepts the matrix. Merely
  ill-conditioned covariances still pass — condition number `1e12` is fine.
