...
* **Test-coverage measurement.** ``pytest-cov`` joins the ``test`` and ``dev``
  ...
  until a ``CODECOV_TOKEN`` secret exists, so nothing leaves the repository
  until public coverage reporting is deliberately switched on. See
  :doc:`installation` for local usage.
